...
summaryStats(IngTotalInd ~ EducaMax, data= (subset(Ind_2019_04_subset[,c("IngTotalInd","EducaMax",
                                                                         "EducaMaxFin" )], 
                                                   EducaMaxFin == "No")),
             stats.in.rows = TRUE, ci=TRUE, conf.level=0.99, p.value = FALSE,
             digits = 1, combine.groups = FALSE)

# Tabla de frecuencia del nivel máximo educativo por sexo
summaryStats(EducaMax ~ Sexo, data= na.omit(Ind_2019_04_subset[,c("EducaMax","Sexo" )]),
             stats.in.rows = TRUE, digits = 1, combine.groups = FALSE)
...
XdataTeor = rnorm(50000,40,5)
Densidad = density(XdataTeor)

Xdata = rnorm(50000,40,5)

HistPlotlyNormal = plot_ly(
  x = Xdata,
  type = "histogram", stroke = I("Black"), nbinsx = 40) %>% layout(yaxis=list(type='linear'))  %>% 
  ...
  layout(xaxis = x, yaxis = y, yaxis2 = list(overlaying = "y", side = "right"),
         title = "Histograma y función de densidad aproximada")
HistPlotlyNormal
...
HistPlotlyTCLn1 = plot_ly(
  x = Xdata,
  type = "histogram", stroke = I("Black"), nbinsx = 20, name="Histograma Normal") %>% layout(yaxis=list(type='linear'))  %>% 
...
datosM3 = (subset(Ind_2019_04_subset[,c("IngTotalInd","EducaMax", "EducaMaxFin","RamaActi","Sexo" )], 
                  RamaActi >=58000 & RamaActi <64000 & Sexo == "Mujer"))

hopotesisIngre6 = t.test(na.omit(datosH3$IngTotalInd),na.omit(datosM3$IngTotalInd), mu = 0, conf.level = Conlevel)
